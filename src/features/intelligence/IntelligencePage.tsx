...
const workSteps: Record<ResponseMode, Array<{ label: string; detail: string }>> = {
  briefing: [
    { label: "Scoping the review", detail: "Meridian Foods and the selected finding" },
    { label: "Reading approved evidence", detail: "6 sources, including the renewed customer contract" },
    { label: "Reconciling the assessment", detail: "Original risk, updated evidence, and open judgment" },
    { label: "Preparing the briefing", detail: "Decision-relevant changes and suggested conditions" },
  ],
  coverage: [
    { label: "Loading the credit model", detail: "Actual, base, and downside cases" },
    { label: "Testing the covenant", detail: "1.25× minimum fixed-charge coverage" },
    { label: "Comparing projected quarters", detail: "Q4 2025 through Q3 2026" },
    { label: "Building the decision view", detail: "Breach timing and distance to covenant" },
  ],
};
...
function IntelligencePageContent() {
  ...
  useEffect(() => {
    ...
    const progressTimers = [
      window.setTimeout(() => setWorkStep(1), 560),
      window.setTimeout(() => setWorkStep(2), 1160),
      window.setTimeout(() => setWorkStep(3), 1780),
      window.setTimeout(() => {
            ...
            <section className={styles.contextGroup} key={category} aria-label={category}>
              ...
              {options.map((context) => {
                ...
                return (
                  ...
                  >
                    <span className={styles.contextIcon}><Icon name={context.icon} size="sm" /></span>
              ...
              <div className={styles.contextChips} aria-label="Selected context">
                {selectedContexts.map((context) => (
                  <span className={styles.contextChip} key={context.id}>
                    <Icon name={context.icon} size="xs" />
            ...
            {turns.map((turn) => {
              ...
              return (
                <article className={styles.turn} key={turn.id}>
                  ...
                  {turn.status === "working" ? (
                    <section className={styles.workTrace} role="status" aria-live="polite" aria-label="Intelligence work in progress">
                      ...
                      <div className={styles.workSteps}>
                        {steps.map((step, index) => (
                          <div className={`${styles.workStep} ${index < workStep ? styles.workStepComplete : ""} ${index === workStep ? styles.workStepActive : ""}`} key={step.label}>
                            <span className={styles.workStepMark} aria-hidden="true">{index < workStep ? <Icon name="check" size="xs" /> : index + 1}</span>
                    ...
                    <>
                      ...
                      <div className={styles.assistantLabel}><span aria-hidden="true">B</span> BCGX Intelligence</div>
                    </>
                  )}
